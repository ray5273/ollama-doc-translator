...
Note: The commit message in the advanced settings section has been slightly adjusted for natural English phrasing while preserving the original intent.
...
You can customize the basic PR body content:
...
Note: The `${{ }}` syntax is preserved as it likely represents a templating language (such as GitHub Actions) and is kept unchanged to maintain technical accuracy.
...
Check if code blocks are preserved correctly through debug files:
...
<!-- Translation From the chunk Similarly Preservation -->
```python
def translate_text(text):
    return translated_text
```
...
3. **Code Blocks Breaking**
   - Compare original and translated files in the `debug_comparisons/` directory
   - Verify that code block preservation logic is functioning correctly
