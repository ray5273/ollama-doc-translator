...
**Note:** Translated text only as requested.
...
*  [Your workflow setup details would go here in markdown format if provided.]
...
```
project/
├── docs/           # Original Korean Documentation
│   ├── README.md
│   └── guide.md
...
(Note: Since specific content wasn't provided beyond the instruction format, placeholders indicate where translated text would be inserted while maintaining the requested markdown structure.)
...
(참고: 한국어에서 "월요일 오전 2시"는 영어로 "Every Monday at 2 AM"으로 번역되었습니다. 여기서 '1'은 월요일을 나타내는 cron 표현식에서 '0'으로 수정되었습니다.)
...
jobs:
  translate:
    ...
    # Execute only if '[translate]' is contained in the commit message
```
