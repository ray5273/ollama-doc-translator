...
<!-- Options -->
<!-- [Proposed | Rejected | Approved | Discarded | … | [ADR-0005](0005-example.md)The provided text "로" translates to "as" or "with" in English, depending on context. Without additional context, a direct translation cannot specify the exact usage. Please provide more context if needed for a precise translation. 
...
- Decision Makers: Kim Min-ho, Lee Ji-young, Park Dong-hwan, Choi Min-je, Jeong Kyung-ho, Nam Seon-seo, Lim Jae-won, Kang Hyun-jin, Seo Min-seok, Oh Yu-jin, Baek Tae-hoon
- Date: 2024-12-17
...
<!-- Decision The basis is Every At every decision Different Water It exists. -->
## Decision Basis
<!-- Decision basis can vary for each decision -->
<!-- [Basis 1, e.g., Performance and Scalability] -->
<!-- [Basis 2, e.g., Advanced Features, Community and Support, Licensing, etc...] -->

- Directly control database configuration without passing through ConfigurationManager, enabling direct control from WebService.
- Previously, database connections were managed through Connection Pool, but with Database Driver usage, endpoints for connection management at the application level are available, thus allowing control without necessarily going through Connection Pool or ConfigurationManager.
...
- To enhance user management convenience, options are provided at a simplified level compared to the database configuration command level, categorized into two forms:
  ...
  | ------------ | ---- | ----------- | ----- |
  | PerformanceMode | HighThroughput | Throughput-centric | Supports up to 1000 connections |
  ...
  | ConnectionState | Specific Connection State | User-defined setting activated when PerformanceMode is selected | - |
...
<!-- [e.g., Learning curve needed, migration required] -->
<!-- [Positive and negative impacts due to this decision] -->

<!-- Example: [ADR-0005](0005-example.md)Row Improved -->
...
- As mentioned in [Decision Rationale](#decision-rationale), the WebService can **directly dynamically control** database connection states.
