...
- When a database supports performance mode but incorrect parameters are provided
  - Sets to the closest possible value.
- During a Hot Swap event for the database, checks the current performance mode and applies necessary adjustments according to configuration if needed.
  ...
  - System-level performance management synchronization and updates related to these changes are required for the database.
...
- [Link Type] [Insert ADR Link Here] <!-- Example: [ADR-0005](0005-example.md) for improvement -->
