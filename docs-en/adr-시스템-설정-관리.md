...
<!-- Options -->
<!-- [Proposed | Rejected | Approved | Discarded | … | [ADR-0005](0005-example.md)The provided text "로" translates to "as" or could be context-dependent, potentially meaning "with" or another preposition depending on usage. More context is needed for accuracy. Assuming a general translation request:
...
- Decision Makers: Kim Min-ho, Lee Ji-young, Park Dong-hwan, Choi Min-soo, Jeong Kyung-ho, Nam Seon-seo, Lim Jae-won, Kang Hyun-jin, Seo Min-seok, Oh Yu-jin, Baek Tae-hun
- Date: 2024-12-17
...
<!-- [Evidence 2, e.g., Luxury Function, Community And Support, License Back...] -->
...
- Currently, when `ConfigurationManager` starts, system settings (including parameters) are loaded from a static configuration file, and there is a requirement that during the initialization of `SystemSetup`, these configurations, particularly database connection settings or cache management, should be dynamically adjustable via REST APIs within the WebService without going through `ConfigurationManager`. This issue needs to be addressed.
...
- To enhance user management convenience, options are provided at a simplified level compared to the database spec configuration commands, categorized into two forms:
  ...
  | ------------ | ---- | ----------- | ----- |
  | PerformanceMode | HighThroughput | Performance throughput-focused | Up to 1000 connections |
  ...
  |             | Manual | Custom connection state defined by user | - |
...
<!-- [Example: Learning curve needed, migration required] -->
<!-- [Pros and Cons resulting from this decision] -->

<!-- Example: [ADR-0005](0005-example.md)The provided text "로" translates to "as" or could be contextually interpreted differently depending on usage (e.g., as a particle indicating manner or purpose). Without additional context, a direct translation is challenging, but generally, it relates to "as" in English. Improved -->
