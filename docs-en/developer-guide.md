...
def count_tokens(text: str) -> int:
    ...
    try:
        ...
        korean_chars = len(re.findall(r'[The provided text "가" is incomplete as a Korean word on its own, typically serving as a particle rather than a standalone word with meaning. Without additional context, a direct translation isn't fully applicable. However, if forced to respond succinctly based on its common usage:

"As" or "Subject marker" (depending on context).-Hehe 😊]', text))
...
def get_translation_prompt(text, source_lang="ko", target_lang="en"):
    prompts = {
        ...
        ("ko", "ja"): f"Next Korean text translation requested but no specific text provided beyond "한국어를". Could you please provide the Korean text you would like translated? It seems there might be a misunderstanding in your request as it abruptly shifts to "일본어로" which means "in Japanese" without providing the Korean text to translate. Could you please provide the Korean text you would like translated into English? Translation: {text}",
