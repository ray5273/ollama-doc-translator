...
def translate_with_ollama(text, model="exaone3.5:7.8b"):
    """
    Ollama APIIt seems like your text might be incomplete. Could you please provide the full Korean text you would like translated? Using Please provide the Korean text you would like translated. Translation
    
    Args:
        text (str): Please provide the Korean text you would like me to translate. Please provide the Korean text you would like translated to English. Please provide the Korean text you would like translated.
...
def split_markdown_by_sections(content: str, max_tokens: int = None) -> list:
    """Section Based Markdown Division - Meaning Unit Preservation"""
    # 1. Heading Hierarchical Structure Analysis (H1-H6)
    # 2. Tracking Code Block Status (``` ~ ``` preserved)
    # Maintain semantic units within token limits.
...
def count_tokens(text: str) -> int:
    ...
    try:
        ...
        korean_chars = len(re.findall(r'[The provided text "가" is incomplete as a Korean word on its own, often serving as a particle rather than a standalone word with a direct translation. However, if contextually intended as a standalone element, it might relate to usage in sentences rather than translating directly. Could you please provide more context or the full sentence for a precise translation?-Hehe 😊]', text))
...
def get_translation_prompt(text, source_lang="ko", target_lang="en"):
    prompts = {
        ...
        ("ko", "ja"): f"Next Korean text translation requested but no specific text provided beyond "한국어를". Could you please provide the Korean text you would like translated? It seems there might be a misunderstanding in your request as it abruptly ends with "일본어로" which means "in Japanese" in English, but no Korean text was provided for translation into Japanese. Could you please provide the Korean text you would like translated? If the intention was to request a translation into Japanese instead, please clarify or provide the Korean text. Translation: {text}",
